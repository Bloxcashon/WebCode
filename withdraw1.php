...
<script>
            ...
            if (xhr.status === 200) {
                ...
                if (gamepassFound) {
                    ...
                    confirmWithdrawalBtn.addEventListener('click', () => {
                        showLoadingOverlay();
                        initiateWithdrawal(gameId, gamepassId, price);
                    });
                } else {
                    Swal.fire({
                        title: 'Error',
  ...
  function showPrivateServerDetails(gameId) {
    ...
    withdrawSteps.innerHTML = `
      ...
      <ol>
        ...
        <li>After creating the private server, click the button below to complete your withdrawal.</li>
      </ol>
      <button id="confirm-withdrawal-btn">Confirm Withdrawal</button>
    `;
    ...
    confirmWithdrawalBtn.addEventListener('click', () => {
      showLoadingOverlay();
      initiateWithdrawal(gameId, null, price);
    });
  }
  ...
  function initiateWithdrawal(gameId, gamepassId, amount) {
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'insert_wstatus.php', true);
    xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');

    const data = `unique_id=${uniqueId}&user_id=${userId}&game_id=${gameId}&gmp_id=${gamepassId}&amount=${amount}`;

    xhr.onload = function() {
      if (xhr.status === 200) {
        const response = JSON.parse(xhr.responseText);
        if (response.txn_id) {
          checkTransaction(response.txn_id);
        } else {
          hideLoadingOverlay();
          Swal.fire({
            title: 'Error',
            text: 'Failed to initiate withdrawal. Please try again.',
            icon: 'error',
            confirmButtonText: 'OK'
          });
        }
  ...
  function check() {
    const xhr = new XMLHttpRequest();
    xhr.open('GET', `check_transaction.php?txn_id=${txnId}`, true);
      ...
      if (xhr.status === 200) {
        try {
          ...
          if (response.status === 'success') {
            ...
            Swal.fire({
              title: 'Success!',
              text: `${response.amount} has been successfully paid out to you, ${response.roblox_username}. Note: It will take around 3-5 days to reflect on your account.`,
              icon: 'success',
            ...
            Swal.fire({
              title: 'Error',
              text: 'We could not process your withdrawal request. Kindly ensure you have created a VIP SERVER for the chosen public game, and set the details as instructed then try again. If this doesn\'t work the VIP Server option might be down so kindly use the GamePass option. Sorry for the inconvenience.',
              icon: 'error',
              confirmButtonText: 'OK'
            });
          } else if (new Date().getTime() - startTime > 180000) { // 3 minutes timeout
            hideLoadingOverlay();
            Swal.fire({
              title: 'Timeout',
              text: 'Withdrawal request failed. Kindly try again!',
              icon: 'error',
              confirmButtonText: 'OK'
            });
          } else {
